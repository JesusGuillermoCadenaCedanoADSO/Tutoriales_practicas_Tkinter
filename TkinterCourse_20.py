...
def update():
    conn_update = sqlite3.connect('address_book.db')
...
def edit():
    ...
    editor.iconbitmap('E:/PycharmProjects/imagenes/file_type_python_icon_130221.ico')
    editor.geometry("400x400")

    conn_editor = sqlite3.connect('address_book.db')
    c_editor = conn_editor.cursor()

    record_id=delete_box.get()

    # query de database

    c_editor.execute("SELECT * FROM addresses WHERE oid =" + record_id)
    records = c_editor.fetchall()
    ...
    edit_btn_editor.grid(row=6,column=0,columnspan=2,pady=10,padx=10,ipadx=145)
